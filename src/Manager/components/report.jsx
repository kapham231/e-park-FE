...
const ReportContent = () => {
    ...
    const renderMaintenance = (selectInvoice) => {
        ...
        return (
            <div>
                <p><strong>ID:</strong> {invoiceNumber}</p>
                <p><strong>Type:</strong> {renderType(__t)}</p>
                <p><strong>Status:</strong> {renderStatus(status)}</p>
                <p><strong>Date:</strong> {createdAt}</p>
                <p><strong>Total items:</strong> {totalItems}</p>
                <p><strong>Amount:</strong> {renderAmount(subtotal)}</p>
                <p><strong>Title:</strong> {title}</p>
                <p><strong>Supplier:</strong> {supplierName}</p>
                <div><strong>Equipments:</strong>
                    <Table
                        dataSource={equipments}
                        columns={[
                            { title: "Code", dataIndex: "code", key: "code", ellipsis: true },
                            { title: "Type", dataIndex: "typeName", key: "typeName", ellipsis: true },
                            { title: "Price", dataIndex: "price", key: "price", ellipsis: true, render: renderInvoiceDetail },
                        ]}
                        rowKey="code"
                        pagination={false}
                        scroll={{
    ...
    const renderBooking = (selectInvoice) => {
        ...
        return (
            <div>
                ...
                <p><strong>Status:</strong> {renderStatus(status)}</p>
                <p><strong>Date:</strong> {createdAt}</p>
                <p><strong>Total tickets:</strong> {totalItems}</p>
                <p><strong>Amount:</strong> {renderAmount(subtotal)}</p>
                ...
                <div><strong>Tickets:</strong>
                    <Table
                        ...
                        columns={[
                            ...
                            { title: "Total", dataIndex: "totalForTicketType", key: "totalForTicketType", render: renderInvoiceDetail },
                        ]}
                        pagination={false}
                        scroll={{
                            x: 'max-content',
    ...
    const totalIncome = filteredTransactions.filter(t => t.subtotal > 0 & t.status === 'PAID').reduce((sum, t) => sum + t.subtotal, 0);
    const totalExpense = filteredTransactions.filter(t => t.subtotal < 0 & t.status === 'PAID').reduce((sum, t) => sum + t.subtotal, 0);
