...
const UserHeader = () => {
  ...
  const activeTab = location.pathname.startsWith("/user/event")
    ? "/user/event"
    : location.pathname;
  ...
  return (
    <>
      <MDBNavbar expand="md" light bgColor="light" fixed>
        <MDBContainer
          fluid
          className="d-flex justify-content-space-between flex-nowrap"
        >
          {!isMobile ? (
          ...
          <div
            className="d-none d-md-flex flex-row ms-3"
            style={{ gap: "24px", flexGrow: 1, paddingLeft: "36px" }}
          >
            <Link
              to="/user/homepage"
              className={`nav-link ${
                activeTab === "/user/homepage" ? "us-active-link" : ""
              }`}
            >
              Homepage
            </Link>
            <Link
              to="/user/event"
              className={`nav-link ${
                activeTab === "/user/event" ? "us-active-link" : ""
              }`}
            >
              Event
            </Link>
            <Link
              to="/user/product"
              className={`nav-link ${
                activeTab === "/user/product" ? "us-active-link" : ""
              }`}
            >
              Product
            </Link>
            <Link
              to="/user/about-us"
              className={`nav-link ${
                activeTab === "/user/about-us" ? "us-active-link" : ""
              }`}
            >
              About Us
            </Link>
            ...
            <>
              ...
              >
                ...
                <DefaultButton
                  ...
                  size="sm"
                  onClick={() =>
                    navigate("/user/register", { state: { user: auth?.user } })
                  }
                >
                  Book Ticket
                </DefaultButton>
          ...
          >
            <div className="d-flex align-items-center">
              ...
              <span style={{ whiteSpace: "nowrap" }}>
                {!!isAuth ? <>Hello {auth?.user?.lastName}</> : "Hello Guest"}
              </span>
            </div>
          </Dropdown>
        </MDBContainer>
