...
import { MDBNavbar, MDBContainer, MDBNavbarBrand, MDBNavbarToggler } from 'mdb-react-ui-kit'
      ...
      <MDBNavbar expand='md' light bgColor='light' fixed>
        <MDBContainer fluid className='d-flex justify-content-space-between flex-nowrap'>
          ...
          <div className='d-md-none' style={{ width: 'min-content' }}>
            <Dropdown menu={{ items: navMenuItems }} placement='bottom' trigger={['click']} arrow>
              <MDBNavbarToggler
          ...
          <div className='d-none d-md-flex flex-row ms-3' style={{ gap: '24px', flexGrow: 1, paddingLeft: '36px' }}>
            <Link to='/user/homepage' className={`nav-link ${activeTab === '/user/homepage' ? 'us-active-link' : ''}`}>
              Homepage
            </Link>
            <Link to='/user/event' className={`nav-link ${activeTab === '/user/event' ? 'us-active-link' : ''}`}>
              Event
            </Link>
          ...
          {isAuthenticated ? (
            ...
            <>
              <DefaultButton size='sm' type='cyan' onClick={() => navigate('/welcome')} style={{ color: 'green' }}>
