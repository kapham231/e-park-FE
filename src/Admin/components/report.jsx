...
const ReportContent = () => {
  ...
  useEffect(() => {
    if (selectedTransaction && (selectedTransaction.__t.toLowerCase() === 'invoiceequipment' || selectedTransaction.__t.toLowerCase() === 'invoiceimportproduct')) {
      const supplierId = selectedTransaction.supplierId || selectedTransaction.supplier
      if (supplierId) {
  ...
  const renderAmount = (amount) => (
    <Text style={{ color: amount > 0 ? 'green' : 'red' }}>
      {amount > 0 ? `+${amount.toLocaleString()} VND` : `${amount.toLocaleString()} VND`}
    </Text>
  ...
  const renderType = (type) => {
    ...
    switch (type) {
      case 'invoiceequipment':
        return <Tag color={'blue'}>Equipment</Tag>
      case 'invoicebooking':
        return <Tag color={'purple'}>Booking</Tag>
      case 'invoiceimportproduct':
        return <Tag color={'orange'}>Import Product</Tag>
      case 'invoiceproduct':
        return <Tag color={'green'}>Sell Product</Tag>
      default:
  ...
  const renderImportProduct = (selectInvoice) => {
    const { invoiceNumber, __t, status, createdAt, totalItems, subtotal, supplierId, products } = selectInvoice
      ...
      <div>
        ...
        <div>
          ...
          <Table
            dataSource={products}
            columns={[
              { title: 'Name', dataIndex: 'typeName', key: 'typeName', ellipsis: true },
              { title: 'Quantity', dataIndex: 'quantity', key: 'quantity' },
              {
                title: 'Price',
                dataIndex: 'price',
  ...
  const renderSellProduct = (selectInvoice) => {
    const { invoiceNumber, __t, status, createdAt, totalItems, subtotal, customer, products } = selectInvoice
    return (
      <div>
        <p>
          <strong>ID:</strong> {invoiceNumber}
  ...
  const columns = [
    ...
    },
    {
      ...
      filters: [
        ...
        { text: 'Equipment', value: 'invoiceequipment' },
        { text: 'Import Product', value: 'invoiceimportproduct' },
        { text: 'Sell Product', value: 'invoiceproduct' }
      ],
  ...
  const filteredTransactions = transactions.filter((t) => {
    ...
    return transactionDate.isBetween(filterRange[0], filterRange[1], undefined, '[]')
  })
  ...
  return (
    <div>
      ...
      />
      <Modal title='Chi Tiết Giao Dịch' centered open={visible} onCancel={() => setVisible(false)} footer={null}>
        {selectedTransaction
          ? selectedTransaction.__t.toLowerCase() === 'invoicebooking'
            ? renderBooking(selectedTransaction)
            : selectedTransaction.__t.toLowerCase() === 'invoiceimportproduct'
            ? renderImportProduct(selectedTransaction)
            : selectedTransaction.__t.toLowerCase() === 'invoiceproduct'
            ? renderSellProduct(selectedTransaction)
