...
export const downloadInvoice = (invoiceData) => {
  ...
  const statusColor =
    invoiceData.status === "PAID" ? [40, 167, 69] : [220, 53, 69]; // Xanh lá hoặc đỏ
  ...
  invoiceData.tickets.forEach((ticket) => {
    ...
    doc.text(
      formatCurrencyForPDF(invoiceData.tickets[0].originalPrice),
      190,
      startY,
      { align: "right" }
    ); // Giá vé
    ...
    doc.text(formatCurrencyForPDF(ticketTotal), 190, startY, {
      align: "right",
    }); // Tổng tiền vé (giá * số lượng)
  ...
  if (invoiceData.tickets[0].bonus > 0) {
    const extraPrice =
      invoiceData.tickets[0].bonusAmount / invoiceData.tickets[0].bonus; // Giá tiền của mỗi extra
  ...
  doc.text(formatCurrencyForPDF(invoiceData.subtotal), 190, startY, {
    align: "right",
  });
  ...
  // Xuất file PDF - Tự động tải về máy
  const pdf = doc.output("blob");
  const url = URL.createObjectURL(pdf);
  const link = document.createElement("a");
  link.href = url;
  link.download = `invoice_${invoiceData.invoiceNumber || new Date().getTime()}.pdf`;
